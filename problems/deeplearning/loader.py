"""Deep Learning items loader - Item schema v1."""
...
class DLItem:
    ...
    problem_type: str = "choice"  # choice|fill|code
    ...
    for p in _load_raw():
        ...
        items.append(
            DLItem(
                ...
                domain=str(p.get("domain", "deeplearning") or "deeplearning"),
                format=str(p.get("format", "dl_choice") or "dl_choice"),
                ...
                problem_type=str(p.get("problem_type", "choice") or "choice"),
...
def get_dl_item_by_id(item_id: str) -> Optional[DLItem]:
    for p in get_dl_items():
        if p.item_id == item_id:
            return p
    return None


def get_dl_items_by_tags(tags: List[str]) -> List[DLItem]:
    if not tags:
        return []
    items = get_dl_items()
